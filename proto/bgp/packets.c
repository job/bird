...
{
  const byte *name;
  byte *t, *t2, argbuf[36], argbuf2[BGP_SHUTDOWN_COMMUNICATION_SIZE];
  unsigned i;
  unsigned len_field;
  ...
  if (len)
    {
      ...
      *t++ = ' ';
      ...
      if ((code == 6) && (subcode == 2))
        {
          len_field = data[0];
          /* Parse a shutdown communication message if first byte equals the length of the rest and the size does not exceed specifications */
          if (len_field <= BGP_SHUTDOWN_COMMUNICATION_SIZE && len == len_field + 1){

            for (i=1; i<len; i++){
              t2 += bsprintf(t2, "%c", data[i]);
            }
          }
        goto done;
        }
  ...
  *t = 0;
  log(L_REMOTE "%s: %s: %s%s%s", p->p.name, msg, name, argbuf, argbuf2);
}
...
bgp_rx_notification(struct bgp_conn *conn, byte *pkt, int len)
{
  ...
  int err = (code != 6);
  if ((code == 6) && (subcode == 2)){
    byte *t = p->shutdown_comm;
    unsigned len_field = pkt[21];
    unsigned i;
    /* Parse a shutdown communication message if first byte equals the length of the rest and the size does not exceed specifications */
    if(len_field <= BGP_SHUTDOWN_COMMUNICATION_SIZE && len - 21 == len_field + 1){
      for (i=22; i<len ; i++){
        t += bsprintf(t, "%c", pkt[i]);
      }
    }
  }
